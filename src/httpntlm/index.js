/**
 * Vendored from httpntlm@1.8.13 (https://github.com/SamDecrock/node-http-ntlm).
 * See ./LICENSE for the original copyright and license.
 *
 * Vendored (rather than depended on) to drop the transitive dependency on
 * `underscore@~1.12.1`, which is vulnerable to CVE-2026-27601. httpntlm has
 * had no release since pinning that range, and npm's `overrides`/`resolutions`
 * fields only apply when the declaring package is the root of the install,
 * so pinning underscore here would not protect anyone using strong-soap as a
 * dependency. `_.omit`/`_.extend` below are now `lodash`, which strong-soap
